...
- **`getKey(Object id)`**: Returns a **stable** `GlobalKey` mapped to the given `id`.
    - **Crucial**: Use this instead of `GlobalKey()` for sub-pickers. `GlobalKey()` creates a new key every build, causing the sub-picker to lose state/close on parent rebuilds. `getKey(id)` returns the cached key for the active popup.
...
- **Keys**: closed pickers should stay cheap. The trigger no longer keeps a permanent `GlobalKey`; only popup/header widgets that need stable identity should use `actions.getKey('unique_id')`.
- **Key lifecycle**: keys returned by `actions.getKey(...)` are managed for the active popup and cleared when it closes, so avoid treating them as long-lived external state.
